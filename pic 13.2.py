...
def draw():
    ...
    sleep(2)
    for k in range(20):
        mushroom(random.randint(0, 400), random.randint(0, 400), random.randint(0, 629)/100, random.randint(10, 200)/100)
        pygame.display.update()
        sleep(2)

def background():
    #this function draws background, x value is the coordinate of the horizon
    x = 250
    e_colour = (64, 64, 64)
    s_colour = (32, 32, 255)
...
def tree(y, x, w):
    #this function draws a tree, y is coordinate of the bottom of the tree
    #x is coordinate of the left side of the tree and w is the width of the tree
    t_colour = (200, 200, 0)
    rect(screen, t_colour, (x, 0, w, y))

def elps(x1, y1, x2, y2, a, cl):
     # this function draws ellipses with focal points at x1, y1 and x2, y2, a  - parameter2 and cl - colour
     for i in range (400):
         for j in range (400):
...
def mushroom (x1, y1, a, s):
    #this function draws mushroom with coordinates x1, y1
    #turned right on a radians and of size s (s=1 is usual size)
...
def body(x, y):
    #this function draws hedgehod's body at x, y
    x -= 200
    y -= 200
...
def spike(x, y):
    #this function draws hedgehod's spikes, where x, y - coordinates of the body
    for i in range(20):
...
def sonic(x, y):
    #this function completes the hedgehog with mushrooms
    body(x, y)
    mushroom(x, y, 1, 0.5)
